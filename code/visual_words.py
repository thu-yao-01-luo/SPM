    ...
    for i in range(len(scales)):
        for c in range(3):
            img = scipy.ndimage.gaussian_filter(
                image[:, :, c], sigma=scales[i])
            imgs.append(img)
        for c in range(3):
            img = scipy.ndimage.gaussian_laplace(
                image[:, :, c], sigma=scales[i])
            imgs.append(img)
        for c in range(3):
            img = scipy.ndimage.gaussian_filter(
                image[:, :, c], sigma=scales[i], order=[0, 1])
            imgs.append(img)
        for c in range(3):
            img = scipy.ndimage.gaussian_filter(
...
def compute_dictionary_one_image(args):
    ...
    * sampled_response: numpy.ndarray of shape (alpha,3F)
    ...
    sampled_response = filter_response.reshape(
        filter_response.shape[0] * filter_response.shape[1], -1)[random_sample, :]
